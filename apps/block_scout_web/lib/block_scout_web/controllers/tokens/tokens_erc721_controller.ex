defmodule BlockScoutWeb.TokensERC721Controller do
  ...
  def index(conn, %{"type" => "JSON"} = params) do
    ...
    paging_params =
      params
      |> paging_options()
      |> Keyword.merge([ token_type: ["ERC-721"] ])

    tokens = Token.list_top(filter, paging_params)

    {tokens_erc721_page, next_page} = split_list_by_page(tokens)

    next_page_path =
      case next_page_params(next_page, tokens_erc721_page, params) do
        nil ->
          nil
  ...
  def index(conn, _params) do
    render(conn, TokensView, "index.html", current_path: Controller.current_full_path(conn), type: "ERC721")
  end
end
