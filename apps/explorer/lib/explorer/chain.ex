defmodule Explorer.Chain do
  ...
  @spec confirmations(Block.t() | nil, [{:block_height, block_height()}]) ::
          {:ok, non_neg_integer()} | {:error, :non_consensus | :not_loaded | :pending}
  ...
  def transactions_available_count do
    Transaction.transactions_available_count()
  end
  ...
  def fetch_recent_collated_transactions(
    ...
    Transaction.recent_collated_transactions(
      ...
      |> Keyword.put(:necessity_by_association, necessity_by_association)
      |> Keyword.put(:method, method_id_filter)
      |> Keyword.put(:type, type_filter)
    )
  end
  ...
  def recent_pending_transactions(options \\ [], old_ui? \\ true)
      when is_list(options) do
    Transaction.recent_pending_transactions(options, old_ui?)
  end
  ...
  def pending_transactions_query(query) do
    Transaction.pending_transactions_query(query)
  end
  ...
  def pending_transactions_list do
    Transaction.pending_transactions_list()
  end
  ...
  def transaction_to_token_transfers(transaction_hash, options \\ []) when is_list(options) do
    ...
    case paging_options do
      ...
      _ ->
        ...
        |> TokenTransfer.filter_by_token_types(token_type)
        |> TokenTransfer.filter_by_token_extension(Keyword.get(options, :token_extension), options)
