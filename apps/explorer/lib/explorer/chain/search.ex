defmodule Explorer.Chain.Search do
  ...
  defp search_contract_query(term) do
    contract_search_fields =
      ...
      |> Map.put(:is_not_contract_address, false)
    ...
    case Chain.string_to_address_hash(term) do
      {:ok, address_hash} ->
        address_search_fields =
          ...
          |> Map.put(:is_verified, dynamic([address, _], address.verified))
          |> Map.put(:is_not_contract_address, dynamic([address, _], is_nil(address.contract_code)))
