...
defmodule Explorer.Chain.TokenTransfer do
  ...
  * `:block_consensus` - Consensus of the block that the transfer took place
  ...
  defp maybe_join_token_for_type_filter(query, _token_types) do
    if DenormalizationHelper.tt_denormalization_finished?() or has_named_binding?(query, :token) do
      query
    else
  ...
  defp token_type_condition([]), do: nil

  defp token_type_condition(token_types) do
    if DenormalizationHelper.tt_denormalization_finished?() do
      dynamic([tt], tt.token_type in ^token_types)
    else
