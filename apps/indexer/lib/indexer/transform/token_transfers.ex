defmodule Indexer.Transform.TokenTransfers do
  ...
  alias Explorer.Chain.ScaledUi.{Events, TransferPairing}
  alias Explorer.Repo
  ...
  defp pair_scaled_ui_events(token_transfers, logs) do
    ...
    {matches, orphan_count} =
      token_transfers
      |> Enum.filter(&(&1.token_type == "ERC-20"))
      |> TransferPairing.pair(ui_events)
    ...
    Enum.map(token_transfers, fn transfer ->
      case Map.fetch(matches, TransferPairing.transfer_key(transfer)) do
  ...
  defp parse_scaled_ui_event(log) do
    case TransferPairing.parse_event(log) do
      ...
      {:error, error} ->
        Logger.error(fn -> ["Unknown TransferWithUIAmount format: #{inspect(log)}", Exception.message(error)] end)
        []
    end
  end
