defmodule Indexer.Fetcher.TokenTest do
  ...
  describe "run/3" do
    ...
    test "falls back to hardcoded Mantle predeploy metadata when on-chain calls revert",
         %{json_rpc_named_arguments: json_rpc_named_arguments} do
      # L2 WETH predeploy on Mantle: name()/symbol()/decimals() return nothing,
      # so the indexer used to mark it cataloged with all nils -> "Unnamed token".
      ...
      if json_rpc_named_arguments[:transport] == EthereumJSONRPC.Mox do
        # MetadataRetriever retries failed contract reads up to 3 times.
        expect(
          EthereumJSONRPC.Mox,
          :json_rpc,
          3,
            ...
            {:ok,
             Enum.map(requests, fn %{id: id, method: "eth_call"} ->
               %{
                 id: id,
                 jsonrpc: "2.0",
                 error: %{code: -32015, data: "0x", message: "execution reverted"}
               }
             end)}
          end
        )
